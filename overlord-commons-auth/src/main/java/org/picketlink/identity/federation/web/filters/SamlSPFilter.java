...
/**
 * A {@link Filter} that implements a service provider for SAML SSO (web profile).
 ...
 * and this filter will be removed when possible.
 *
 * @author eric.wittmann@redhat.com
 */
public class SamlSPFilter implements Filter {

    private static Logger log = Logger.getLogger(SamlSPFilter.class);
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if (!postMethod && !logOutRequest) {
            ...
            if (isNotNull(samlRequest)) {
                ...
                // deal with SAML response from IDP
                byte[] base64DecodedRequest = SamlPostBindingUtil.base64Decode(samlRequest);
    ...
    protected void sendRequestToIDP(AuthnRequestType authnRequest, String relayState, HttpServletResponse response)
        ...
        String samlMessage = SamlPostBindingUtil.base64Encode(baos.toString());
        String destination = authnRequest.getDestination().toASCIIString();
        SamlPostBindingUtil.sendPost(new DestinationInfoHolder(destination, samlMessage, relayState), response, true);
    }
